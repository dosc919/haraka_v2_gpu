...
typedef struct crypt_parameters cuda_crypt_parameters;
...
void AES_cuda_crypt(cuda_crypt_parameters *c) {
	...
	_CUDA(cudaMalloc((void**)&c->d_in, c->nbytes*sizeof(char)));
	_CUDA(cudaMemcpyAsync((void *)c->d_in, c->in, c->nbytes, cudaMemcpyHostToDevice, 0));
...
int main()
{
	...
	uint8_t userKey[AES_KEY_SIZE_128] = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
	AES_KEY *key = new AES_KEY;
	int bits = 128;
	AES_cuda_set_encrypt_key(userKey, bits, key);
	AES_cuda_transfer_key_schedule(key);

	cuda_crypt_parameters *c_param = new cuda_crypt_parameters;
	//uint8_t input[AES_BLOCK_SIZE] = { 0x4E, 0xC1, 0x37, 0xA4, 0x26, 0xDA, 0xBF, 0x8A, 0xA0, 0xBE, 0xB8, 0xBC, 0x0C, 0x2B, 0x89, 0xD6 };
	//uint8_t input[AES_BLOCK_SIZE] = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
	uint8_t input[AES_BLOCK_SIZE] = { 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };

	c_param->in = new uint8_t[AES_BLOCK_SIZE];
	for (int i = 0; i < AES_BLOCK_SIZE; i++)
		c_param->in[i] = input[i];
	c_param->nbytes = AES_BLOCK_SIZE;

	printf("\ninput : ");
	for (int i = 0; i < AES_BLOCK_SIZE; i++)
		printf("%x ", c_param->in[i]);

	c_param->out = NULL;

	AES_cuda_crypt(c_param);

	printf("\noutput: ");
	if (c_param->out != NULL)
	for (int i = 0; i < AES_BLOCK_SIZE; i++)
		printf("%x ", c_param->out[i]);
