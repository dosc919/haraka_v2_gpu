...
int testFunctionality(vector<string>& errors)
{
	ifstream file;
	for (auto file_name : TEST_FILES)
	{
		...
		if (!memcmp(&digest[0], &digest_ref[0], digest.size()))
...
int main()
{
	...
	int error = testFunctionality(error_msgs);
	...
	if (!test.good())
		cout << "open error motherfucker" << endl; while (1){}
	...
	if (test.bad() || test.fail())
		cout << "write error motherfucker" << endl;
